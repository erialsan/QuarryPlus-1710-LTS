...
public class RefineryRecipeHelper implements IFlexibleCrafter {
    ...
    public static void get(final TileRefinery tr) {
        if (!ModAPIManager.INSTANCE.hasAPI("BuildCraftAPI|recipes") || tr.cached != null) return;
        ...
        for (final IFlexibleRecipe<FluidStack> ifr : irrm.getRecipes()) {
            ...
            if (cr == null || !check(cr.crafted, tr)) continue;
    ...
    private static boolean check(final FluidStack fs, final TileRefinery tr) {
        final Byte i = tr.get()
            .get(Integer.valueOf(Enchantment.efficiency.effectId));
        return tr.res == null
            || tr.res.isFluidEqual(fs) && tr.buf - tr.res.amount >= fs.amount * (i == null ? 0 : i.byteValue() + 1);
    }
}
