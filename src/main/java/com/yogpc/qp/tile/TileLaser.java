...
public class TileLaser extends APowerTile implements IEnchantableTile {
    ...
    public void updateEntity() {
        ...
        if (!isValidTable() && this.worldObj.getWorldTime() % 100 == this.from % 100) findTable();
        ...
        if (!isValidLaser()) for (int i = 0; i < this.lasers.length; i++) {
        ...
        if (isValidLaser() && this.worldObj.getWorldTime() % 10 == this.from % 10)
            for (int i = 0; i < this.laserTargets.size(); i++) {
                this.lasers[i].x = ILaserTargetHelper.getXCoord(this.laserTargets.get(i)) + 0.475
                    + (this.worldObj.rand.nextFloat() - 0.5) / 5F;
                this.lasers[i].y = ILaserTargetHelper.getYCoord(this.laserTargets.get(i)) + 9F / 16F;
                this.lasers[i].z = ILaserTargetHelper.getZCoord(this.laserTargets.get(i)) + 0.475
                    + (this.worldObj.rand.nextFloat() - 0.5) / 5F;
            }

        final double power = PowerManager
            .useEnergyL(this, this.unbreaking, this.fortune, this.silktouch, this.efficiency);
        ...
        if (this.worldObj.getWorldTime() % 20 == 7) PacketHandler.sendPacketToAround(
            new YogpstopPacket(this),
    ...
    private boolean isValidLaser() {
        for (final Position laser : this.lasers) if (laser == null) return false;
        return true;
    }

    protected boolean isValidTable() {
        if (this.laserTargets.size() == 0) return false;
        for (final Object lt : this.laserTargets) if (lt == null || !ILaserTargetHelper.isValid(lt)) return false;
        return true;
    }
    ...
    protected void findTable() {
        ...
        for (int x = minX; x <= maxX; ++x) for (int y = minY; y <= maxY; ++y) for (int z = minZ; z <= maxZ; ++z) {
            ...
            if (ILaserTargetHelper.isInstance(tile)) if (ILaserTargetHelper.isValid(tile)) this.laserTargets.add(tile);
        }
        if (this.laserTargets.isEmpty()) return;
        if (!this.silktouch) {
            final Object laserTarget = this.laserTargets.get(this.worldObj.rand.nextInt(this.laserTargets.size()));
            this.laserTargets.clear();
    ...
    protected void removeLaser() {
        if (this.lasers != null) for (int i = 0; i < this.lasers.length; i++) this.lasers[i] = null;
        if (!this.worldObj.isRemote) PacketHandler.sendPacketToAround(
            new YogpstopPacket(this),
            this.worldObj.provider.dimensionId,
            this.xCoord,
            this.yCoord,
    ...
    private void pushPower(final double received) {
        ...
        if (this.pi == this.tp.length) this.pi = 0;
    }
    ...
    public ResourceLocation getTexture() {
        ...
        if (avg <= 1.0) return LASER_TEXTURES[0];
        else if (avg <= 2.0) return LASER_TEXTURES[1];
        else if (avg <= 3.0) return LASER_TEXTURES[2];
        else return LASER_TEXTURES[3];
    }
    ...
    public void writeToNBT(final NBTTagCompound nbttc) {
        ...
        if (this.lasers != null) for (final Position l : this.lasers) if (l != null) {
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        ...
        if (this.lasers != null) for (final Position p : this.lasers) {
            if (p == null) continue;
