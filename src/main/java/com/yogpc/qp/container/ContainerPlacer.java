...
public class ContainerPlacer extends Container {
    ...
    public ContainerPlacer(final IInventory par1IInventory, final TilePlacer par2TileEntityDispenser) {
        ...
        for (row = 0; row < 3; ++row) for (col = 0; col < 3; ++col)
            addSlotToContainer(new Slot(par2TileEntityDispenser, col + row * 3, 62 + col * 18, 17 + row * 18));

        for (row = 0; row < 3; ++row) for (col = 0; col < 9; ++col)
            addSlotToContainer(new Slot(par1IInventory, col + row * 9 + 9, 8 + col * 18, 84 + row * 18));

        for (col = 0; col < 9; ++col) addSlotToContainer(new Slot(par1IInventory, col, 8 + col * 18, 142));
    }
    ...
    public ItemStack transferStackInSlot(final EntityPlayer ep, final int i) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            src = remain.copy();
            if (i < 9) {
                if (!mergeItemStack(remain, 9, 45, true)) return null;
            } else if (!mergeItemStack(remain, 0, 9, false)) return null;
