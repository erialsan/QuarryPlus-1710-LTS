...
public class RenderLaser extends TileEntitySpecialRenderer {
    ...
    static void renderLaser(final TextureManager tm, final double fx, final double fy, final double fz, final double tx,
        final double ty, final double tz, final int b, final ResourceLocation tex) {
        GL11.glPushMatrix();
    ...
    public void renderTileEntityAt(final TileEntity te, final double x, final double y, final double z, final float f) {
        ...
        if (laser != null && laser.lasers != null) {
            ...
            for (final TileLaser.Position l : laser.lasers) {
                final ForgeDirection fd = ForgeDirection.values()[te.getWorldObj()
                    .getBlockMetadata(te.xCoord, te.yCoord, te.zCoord)];
                renderLaser(
                    this.field_147501_a.field_147553_e,
                    te.xCoord + 0.5 + 0.3 * fd.offsetX,
                    te.yCoord + 0.5 + 0.3 * fd.offsetY,
                    te.zCoord + 0.5 + 0.3 * fd.offsetZ,
                    l.x,
                    l.y,
                    l.z,
                    (int) (laser.getWorldObj()
                        .getWorldTime() % 40),
                    laser.getTexture());
            }
            GL11.glPopAttrib();
