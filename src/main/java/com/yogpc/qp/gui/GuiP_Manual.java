...
public class GuiP_Manual extends GuiScreenA implements GuiYesNoCallback {
        ...
        switch (par1.id) {
            case -1:
                ...
                if (this.tile.mapping[this.targetid].contains(name)) {
                    ...
                    this.mc
                        .displayGuiScreen(new GuiError(this, StatCollector.translateToLocal("tof.alreadyerror"), name));
                    return;
                }
                if (FluidRegistry.isFluidRegistered(name)) name = FluidRegistry.getFluid(name)
                    .getLocalizedName(FluidRegistry.getFluidStack(name, 0));
                this.mc
                    .displayGuiScreen(new GuiYesNo(this, StatCollector.translateToLocal("tof.addfluidsure"), name, -1));
                break;
            case -2:
                showParent();
                break;
    ...
    public void confirmClicked(final boolean par1, final int par2) {
        if (par1) PacketHandler
            .sendPacketToServer(this.tile, PacketHandler.CtS_ADD_MAPPING, this.targetid, this.blockid.getText());
        else showParent();
    }

    @Override
    protected void keyTyped(final char par1, final int par2) {
        if (this.blockid.isFocused()) this.blockid.textboxKeyTyped(par1, par2);
    ...
    public void drawScreen(final int i, final int j, final float k) {
        ...
        drawCenteredString(
            ...
            StatCollector.translateToLocal("tof.selectfluid"),
            this.width / 2,
            8,
            0xFFFFFF);
        this.fontRendererObj.drawStringWithShadow(
            StatCollector.translateToLocal("tof.fluidid"),
            this.width / 2 - 60 - this.fontRendererObj.getStringWidth(StatCollector.translateToLocal("tof.fluidid")),
            50,
            0xFFFFFF);
