...
public class GuiP_SelectBlock extends GuiScreenA {
    ...
    public void initGui() {
        super.initGui();
        this.blocks = new GuiP_SlotBlockList(
            this.mc,
            this.width,
            this.height,
            24,
    ...
    public void actionPerformed(final GuiButton par1) {
        switch (par1.id) {
            case -1:
                PacketHandler
                    .sendPacketToServer(this.tile, PacketHandler.CtS_ADD_MAPPING, this.targetid, this.blocks.current);
                break;
            case -2:
                showParent();
                break;
    ...
    public void drawScreen(final int i, final int j, final float k) {
        ...
        this.fontRendererObj
            .drawStringWithShadow(title, (this.width - this.fontRendererObj.getStringWidth(title)) / 2, 8, 0xFFFFFF);
