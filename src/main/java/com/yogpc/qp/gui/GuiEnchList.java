...
public class GuiEnchList extends GuiContainer implements GuiYesNoCallback {
    ...
    public void initGui() {
        ...
        this.buttonList.add(
            new GuiButton(PacketHandler.CtS_TOGGLE_FORTUNE + this.targetid, this.width * 2 / 3 + 10, 140, 100, 20, ""));
        this.buttonList.add(
            new GuiButton(
                PacketHandler.CtS_REMOVE_FORTUNE + this.targetid,
                this.width * 2 / 3 + 10,
                110,
                100,
                20,
                StatCollector.translateToLocal("selectServer.delete")));
        this.slot = new GuiSlotEnchList(
            this.mc,
            this.width * 3 / 5,
            this.height - 60,
            30,
        ...
        switch (par1.id) {
            ...
            case PacketHandler.CtS_REMOVE_SILKTOUCH:
                this.mc.displayGuiScreen(
                    new GuiYesNo(
                        this,
                        StatCollector.translateToLocal("tof.deleteblocksure"),
                        QuarryPlusI.getLocalizedName(
                            (this.targetid == 0 ? this.tile.fortuneList : this.tile.silktouchList)
                                .get(this.slot.currentore)),
                        par1.id));
                break;
            default:
    ...
    protected void drawGuiContainerForegroundLayer(final int i, final int j) {
        drawCenteredString(
            ...
            StatCollector.translateToLocalFormatted(
                "qp.list.setting",
                StatCollector
                    .translateToLocal(this.targetid == 0 ? "enchantment.lootBonusDigger" : "enchantment.untouching")),
            this.xSize / 2,
            8,
            0xFFFFFF);
    }
    ...
    public void updateScreen() {
        super.updateScreen();
        ((GuiButton) this.buttonList.get(1)).displayString = StatCollector
            .translateToLocal(include() ? "tof.include" : "tof.exclude");
        ((GuiButton) this.buttonList.get(2)).enabled = !(this.targetid == 0 ? this.tile.fortuneList
            : this.tile.silktouchList).isEmpty();
    }
}
