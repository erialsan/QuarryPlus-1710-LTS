...
public class ContainerWorkbench extends Container implements IPacketContainer {
    ...
    private static final void writeItemStack(final ItemStack is, final DataOutputStream bb) throws IOException {
        if (is == null) {
    ...
    public ContainerWorkbench(final IInventory pi, final TileWorkbench tw) {
        ...
        for (col = 0; col < 9; ++col) addSlotToContainer(new Slot(pi, col, 8 + col * 18, 198));
    }
    ...
    public ItemStack transferStackInSlot(final EntityPlayer ep, final int i) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            src = remain.copy();
            if (i < 27) {
                if (!mergeItemStack(remain, 45, 81, true)) return null;
            } else if (!mergeItemStack(remain, 0, 27, false)) return null;
            if (remain.stackSize == 0) slot.putStack((ItemStack) null);
            else slot.onSlotChanged();
            if (remain.stackSize == src.stackSize) return null;
    ...
    public void addCraftingToCrafters(final ICrafting p_75132_1_) {
        if (this.crafters.contains(p_75132_1_)) throw new IllegalArgumentException("Listener already listening");
    ...
    public void detectAndSendChanges() {
        try {
            ...
            if (dos.size() > 0) {
                dos.writeByte(-1);
                for (int j = 0; j < this.crafters.size(); ++j) PacketHandler.sendPacketToPlayer(
                    new YogpstopPacket(this, bos.toByteArray()),
    ...
    public void receivePacket(final byte[] ba) throws IOException {
        ...
        byte slot;
        while ((slot = bb.readByte()) >= 0) if (slot == 101) this.tile.cpower = bb.readShort();
        else if (slot == 102) this.tile.cur_recipe = bb.readByte();
        else getSlot(slot).putStack(readItemStack(bb));
        return;
    }
    ...
    public static boolean canDrag(final Slot slot, final ItemStack is, final boolean always_true) {
        ...
        if (slot != null && slot.getHasStack()
            && is != null
            && is.isItemEqual(slot.getStack())
            && ItemStack.areItemStackTagsEqual(slot.getStack(), is)) {
            final int i = always_true ? 0 : is.stackSize;
            can |= slot.getStack().stackSize + i
                <= (slot instanceof SlotUnlimited ? slot.getSlotStackLimit() : is.getMaxStackSize());
        }
        return can;
    }
    ...
    public ItemStack slotClick(final int i, final int param, final int type, final EntityPlayer ep) {
        ...
        if (type == 5) {
            ...
            if ((prev_drag_state != 1 || this.drag_state != 2) && prev_drag_state != this.drag_state) func_94533_d();
            else if (player_inv.getItemStack() == null) func_94533_d();
                ...
                if (slot != null && canDrag(slot, player_inv.getItemStack(), true)
                    ...
                    && canDragIntoSlot(slot)) this.dragged.add(slot);
            } else if (this.drag_state == 2) {
                if (!this.dragged.isEmpty()) {
                    ItemStack player_stack = player_inv.getItemStack()
                        .copy();
                    ...
                    if (player_stack.stackSize <= 0) player_stack = null;
                    player_inv.setItemStack(player_stack);
                }
                func_94533_d();
            } else func_94533_d();
        } else if (this.drag_state != 0) func_94533_d();
            ...
            if (i == -999) {
                if (player_inv.getItemStack() != null && i == -999) {
                    ...
                    if (param == 1) {
                        ep.dropPlayerItemWithRandomChoice(
                            player_inv.getItemStack()
                                .splitStack(1),
                            true);
                        if (player_inv.getItemStack().stackSize == 0) player_inv.setItemStack(null);
                    }
                }
            } else if (type == 1) {
                if (i < 0) return null;
                ...
                if (slot != null && slot.canTakeStack(ep)) {
                    ...
                    if (itemstack3 != null) {
                        ...
                        ret = itemstack3.copy();
                        if (slot.getStack() != null && slot.getStack()
                            .getItem() == item) retrySlotClick(i, param, true, ep);
                    }
                }
            } else {
                if (i < 0) return null;
                ...
                if (slot != null) {
                    ...
                    if (slot_stack != null) ret = slot_stack.copy();
                    if (slot_stack == null) {
                        if (player_stack != null && slot.isItemValid(player_stack)) {
                            int l1 = param == 0 ? player_stack.stackSize : 1;
                            if (l1 > slot.getSlotStackLimit()) l1 = slot.getSlotStackLimit();
                            if (player_stack.stackSize >= l1) slot.putStack(player_stack.splitStack(l1));
                            if (player_stack.stackSize == 0) player_inv.setItemStack(null);
                        }
                    } else if (slot.canTakeStack(ep)) if (player_stack == null) {
                        final int l1 = param == 0 ? slot_stack.stackSize : (slot_stack.stackSize + 1) / 2;
                        player_inv.setItemStack(slot.decrStackSize(l1));
                        if (slot_stack.stackSize == 0) slot.putStack(null);
                    ...
                    } else if (slot_stack.getItem() == player_stack.getItem() && player_stack.getMaxStackSize() > 1
                        && (!slot_stack.getHasSubtypes() || slot_stack.getItemDamage() == player_stack.getItemDamage())
                        && ItemStack.areItemStackTagsEqual(slot_stack, player_stack)) {
            ...
            if (slot.canTakeStack(ep)) {
                ...
                boolean can2player = player_stack == null
                    || slot.inventory == player_inv && slot.isItemValid(player_stack);
                int first_empty = -1;
                if (!can2player) {
                ...
                if (slot.getHasStack() && can2player) {// to player
                    ...
                    if ((slot.inventory != player_inv || !slot.isItemValid(player_stack)) && player_stack != null) {// prev2player
                        if (first_empty > -1) {
        ...
        } else if (type == 3 && ep.capabilities.isCreativeMode && player_inv.getItemStack() == null && i >= 0) {
            final Slot slot = (Slot) this.inventorySlots.get(i);
            if (slot != null && slot.getHasStack()) {
                final ItemStack player_stack = slot.getStack()
                    .copy();
    ...
    protected boolean mergeItemStack(final ItemStack is, final int from, final int to, final boolean invert) {
        ...
        if (is.isStackable()) while (is.stackSize > 0 && (!invert && k < to || invert && k >= from)) {
            ...
            if (slot_stack != null && slot_stack.getItem() == is.getItem()
                ...
                if (total > slot.getSlotStackLimit()) total = slot.getSlotStackLimit();
        ...
        while (is.stackSize > 0 && (!invert && k < to || invert && k >= from)) {
            ...
            if (slot.getStack() == null) {
                ...
                if (!(slot instanceof SlotUnlimited) && slot_stack.stackSize > slot_stack.getMaxStackSize())
                    slot_stack.stackSize = slot_stack.getMaxStackSize();
                if (slot_stack.stackSize > slot.getSlotStackLimit()) slot_stack.stackSize = slot.getSlotStackLimit();
                slot.putStack(slot_stack);
                slot.onSlotChanged();
