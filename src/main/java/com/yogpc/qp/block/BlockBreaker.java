...
public class BlockBreaker extends ADismCBlock {
    ...
    public IIcon getIcon(final int par1, final int par2) {
        ...
        return par1 == k ? k != 1 && k != 0 ? this.horizontal : this.vectrial
            : k != 1 && k != 0 ? par1 != 1 && par1 != 0 ? this.blockIcon : this.furnaceTopIcon : this.furnaceTopIcon;
    }
    ...
    @Override
    public void onNeighborBlockChange(final World par1World, final int x, final int y, final int z, final Block b) {
        final boolean flag = par1World.isBlockIndirectlyGettingPowered(x, y, z)
        ...
        } else if (!flag && flag1) par1World.setBlockMetadataWithNotify(x, y, z, i1 & -9, 4);
    }
    ...
    public void updateTick(final World w, final int x, final int y, final int z, final Random r) {
        ...
        final int tx = x + fd.offsetX, ty = y + fd.offsetY, tz = z + fd.offsetZ, meta = w.getBlockMetadata(tx, ty, tz);
        if (ty < 1) return;
        ...
        if (b.canSilkHarvest(w, player, tx, ty, tz, meta) && tile.silktouch) {
            ...
            final ItemStack is = (ItemStack) ReflectionHelper
                .invoke(TileBasic.createStackedBlock, b, new Integer(meta));
            if (is != null) alis.add(is);
        } else alis = b.getDrops(w, tx, ty, tz, meta, tile.fortune);
    ...
    @Override
    public void onBlockPlacedBy(final World world, final int x, final int y, final int z, final EntityLivingBase ent,
        final ItemStack is) {
    ...
    @Override
    public boolean onBlockActivated(final World world, final int x, final int y, final int z, final EntityPlayer ep,
        final int side, final float par7, final float par8, final float par9) {
        final Item equipped = ep.getCurrentEquippedItem() != null ? ep.getCurrentEquippedItem()
            .getItem() : null;
        ...
        if (equipped instanceof ItemTool && ep.getCurrentEquippedItem()
            .getItemDamage() == 0) {
            if (!world.isRemote) for (final IChatComponent s : EnchantmentHelper
                .getEnchantmentsChat((IEnchantableTile) world.getTileEntity(x, y, z))) ep.addChatMessage(s);
            return true;
        }
        return false;
    }
