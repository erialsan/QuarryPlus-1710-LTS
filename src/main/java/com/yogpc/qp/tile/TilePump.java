...
public class TilePump extends APacketTile implements IFluidHandler, IEnchantableTile {
    ...
    public TileBasic G_connected() {
        final TileEntity te = this.worldObj.getTileEntity(
            this.xCoord + this.connectTo.offsetX,
            this.yCoord + this.connectTo.offsetY,
            this.zCoord + this.connectTo.offsetZ);
    ...
    public void readFromNBT(final NBTTagCompound nbttc) {
        ...
        if (nbttc.getTag("mapping0") instanceof NBTTagList)
            for (int i = 0; i < this.mapping.length; i++) readStringCollection(
                nbttc.getTagList(
                    new StringBuilder().append("mapping")
                        .append(i)
                        .toString(),
                    8),
                this.mapping[i]);
    ...
    private static void readStringCollection(final NBTTagList nbttl, final Collection<String> target) {
        target.clear();
        for (int i = 0; i < nbttl.tagCount(); i++) target.add(nbttl.getStringTagAt(i));
    }
    ...
    public void writeToNBT(final NBTTagCompound nbttc) {
        ...
        for (int i = 0; i < this.mapping.length; i++) nbttc.setTag(
            new StringBuilder().append("mapping")
                .append(i)
                .toString(),
        ...
        if (this.silktouch) {
            ...
            for (final FluidStack l : this.liquids) nbttl.appendTag(l.writeToNBT(new NBTTagCompound()));
    ...
    private static NBTTagList writeStringCollection(final Collection<String> target) {
        final NBTTagList nbttl = new NBTTagList();
        for (final String l : target) nbttl.appendTag(new NBTTagString(l));
        return nbttl;
    }
    ...
    public void updateEntity() {
        ...
        for (final ForgeDirection fd : ForgeDirection.VALID_DIRECTIONS) {
            te = this.worldObj
                .getTileEntity(this.xCoord + fd.offsetX, this.yCoord + fd.offsetY, this.zCoord + fd.offsetZ);
    ...
    public void C_recievePacket(final byte id, final byte[] data, final EntityPlayer ep) {
        ...
        switch (id) {
            ...
            case PacketHandler.StC_OPENGUI_MAPPING:// BI[Ljava.lang.String;
                ...
                this.mapping[target].clear();
                for (int i = 0; i < len; i++) this.mapping[target].add(badi.readUTF());
                ep.openGui(
                    QuarryPlus.I,
                    QuarryPlusI.guiIdPump + target,
                    this.worldObj,
                    this.xCoord,
                    this.yCoord,
                    this.zCoord);
    ...
    public void S_OpenGUI(final int d, final EntityPlayer ep) {// BI[Ljava.lang.String;
        try {
            ...
            for (final String s : this.mapping[d]) dos.writeUTF(s);
            PacketHandler
                .sendPacketToPlayer(new YogpstopPacket(bos.toByteArray(), this, PacketHandler.StC_OPENGUI_MAPPING), ep);
    ...
    public void S_changeRange(final EntityPlayer ep) {
        ...
        else ep.addChatMessage(
            new ChatComponentTranslation("chat.pump_rtoggle.num", Integer.toString(this.range * 2 + 1)));
        this.fwt = 0;
    }
    ...
    private void S_searchLiquid(final int x, final int y, final int z) {
        ...
        for (kx = 0; kx < chunk_side_x; kx++)
            for (kz = 0; kz < chunk_side_z; kz++) this.ebses[kx][kz] = this.worldObj.getChunkProvider()
    ...
    boolean S_removeLiquids(final APowerTile tbpp, final int x, final int y, final int z) {
        ...
        if (this.cx != x || this.cy != y
            || this.cz != z
            || this.py < this.cy
            || this.worldObj.getWorldTime() - this.fwt > 200) S_searchLiquid(x, y, z);
        else {
            this.ebses = new ExtendedBlockStorage[this.ebses.length][this.ebses[0].length][];
            int kx, kz;
            for (kx = 0; kx < this.ebses.length; kx++)
                for (kz = 0; kz < this.ebses[0].length; kz++) this.ebses[kx][kz] = this.worldObj.getChunkProvider()
                    .loadChunk(kx + (this.xOffset >> 4), kz + (this.zOffset >> 4))
                    .getBlockStorageArray();
        }
        int count = 0;
        ...
        do {
            do {
                if (this.px == -1) {
                    int bx;
                    for (bx = 0; bx < this.block_side_x; bx++) for (bz = 0; bz < this.block_side_z; bz++)
                        if ((this.blocks[this.py - this.yOffset][bx][bz] & 0x40) != 0) {
                            bb = this.ebses[bx >> 4][bz >> 4][this.py >> 4]
                                .getBlockByExtId(bx & 0xF, this.py & 0xF, bz & 0xF);
                    ...
                    if (this.blocks[this.py - this.yOffset][this.px][bz] != 0) {
                        bb = this.ebses[this.px >> 4][bz >> 4][this.py >> 4]
                            .getBlockByExtId(this.px & 0xF, this.py & 0xF, bz & 0xF);
                        meta = this.ebses[this.px >> 4][bz >> 4][this.py >> 4]
                            .getExtBlockMetadata(this.px & 0xF, this.py & 0xF, bz & 0xF);
        ...
        if (count > 0 && PowerManager.useEnergyP(tbpp, this.unbreaking, count, this.px == -1 ? count : 0))
            if (this.px == -1) {
                int bx;
                for (bx = 0; bx < this.block_side_x; bx++) for (bz = 0; bz < this.block_side_z; bz++)
                    if ((this.blocks[this.py - this.yOffset][bx][bz] & 0x40) != 0) drainBlock(bx, bz, Blocks.glass);
            } else for (bz = 0; bz < this.block_side_z; bz++)
    ...
    {
        for (int i = 0; i < this.mapping.length; i++) this.mapping[i] = new LinkedList<String>();
    }
    ...
    public IChatComponent[] C_getNames() {
        ...
        if (this.liquids.size() > 0) {
            ...
            for (int i = 0; i < this.liquids.size(); i++) {
                ...
                ret[i + 1] = new ChatComponentTranslation(
                    "yog.pump.liquid",
                    new ChatComponentTranslation(
                        s.getFluid()
                            .getUnlocalizedName(s)),
     ...
     * 
     */
    static final boolean isLiquid(final Block b, final boolean s, final World w, final int x, final int y, final int z,
        final int m) {
    ...
    private final void drainBlock(final int bx, final int bz, final Block tb) {
        ...
        final int meta = this.ebses[bx >> 4][bz >> 4][this.py >> 4]
            .getExtBlockMetadata(bx & 0xF, this.py & 0xF, bz & 0xF);
        if (isLiquid(b, false, null, 0, 0, 0, 0)) {
            FluidStack fs = null;
            if (b instanceof IFluidBlock
                && ((IFluidBlock) b).canDrain(this.worldObj, bx + this.xOffset, this.py, bz + this.zOffset))
