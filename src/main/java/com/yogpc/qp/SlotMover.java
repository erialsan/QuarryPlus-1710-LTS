    ...
    public boolean isItemValid(final ItemStack is) {
        switch (this.slotNumber) {
            case 0:
                ...
                if (nbttl != null && (is.getItem() instanceof net.minecraft.item.ItemTool
                    && ((net.minecraft.item.ItemTool) is.getItem()).getToolMaterialName()
                        == Item.ToolMaterial.EMERALD.toString()
                    || is.getItem() instanceof ItemBow)) return true;
                return false;
            case 1:
                if (is.getItem() instanceof IEnchantableItem) return true;
        }
        return false;
    }
