...
public class ItemTool extends Item implements IEnchantableItem {
    ...
    @Override
    public boolean onItemUse(final ItemStack is, final EntityPlayer ep, final World w, final int x, final int y,
        final int z, final int side, final float par8, final float par9, final float par10) {
        if (is.getItemDamage() == 1) {
            boolean s = false, f = false;
            final NBTTagList nbttl = is.getEnchantmentTagList();
            if (nbttl != null) for (int i = 0; i < nbttl.tagCount(); i++) {
                final short id = nbttl.getCompoundTagAt(i)
                    .getShort("id");
                if (id == Enchantment.silkTouch.effectId) s = true;
                if (id == Enchantment.fortune.effectId) f = true;
            }
            ...
            if (b != null && !b.isAir(w, x, y, z)) {
                ...
                final String name = GameData.getBlockRegistry()
                    .getNameForObject(b);
