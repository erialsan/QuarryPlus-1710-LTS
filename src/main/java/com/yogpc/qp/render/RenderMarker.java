...
public class RenderMarker implements ISimpleBlockRenderingHandler {
    ...
    private RenderMarker() {
        ...
        final double[][] frontXBase = { { -0.0625, -0.0625, -0.0625, -0.0625 }, { 1, 0, 0, 1 },
            { -0.5, -0.5, 0.5, 0.5 } };
        ...
        final double[][] frontZBase = { { -0.5, -0.5, 0.5, 0.5 }, { 1, 0, 0, 1 }, { 0.0625, 0.0625, 0.0625, 0.0625 } };
        ...
        final double[][] frontYBase = { { -0.5, -0.5, 0.5, 0.5 }, { -0.0625, -0.0625, -0.0625, -0.0625 },
            { 0.5, -0.5, -0.5, 0.5 } };
    ...
    private static double[][] safeClone(final double[][] d) {
        ...
        for (int i = 0; i < d.length; ++i) for (int j = 0; j < d[0].length; ++j) ret[i][j] = d[i][j];
    ...
    private static void renderMarkerWithMeta(final IBlockAccess iblockaccess, final Block block, final double xi,
        final double yi, final double zi, final int meta) {
        final Tessellator tessellator = Tessellator.instance;
        int metadata = meta;

        if (metadata > 5 || metadata < 0) metadata = 1;
