...
public class WorkbenchRecipe {
    ...
    public static final class WBIS {
        ...
        static WBIS[] convert(final ItemStack[] s) {
            ...
            for (int i = 0; i < s.length; i++) r[i] = new WBIS(s[i]);
            return r;
        }
    ...
    public boolean check(final ItemStack[] sinv) {
        for (final WBIS raw : this.input) {
            ...
            for (int i = 0; i < 27; i++) {
                ...
                if (is.getItem() != raw.item || is.getItemDamage() != raw.meta) continue;
                if (is.stackTagCompound == null && raw.tag != null ? true
                    : is.stackTagCompound != null && !is.stackTagCompound.equals(raw.tag)) continue;
    ...
    public static int getRecipes(final ItemStack[] inv, final WorkbenchRecipe[] rs, final int p) {
        ...
        for (final WorkbenchRecipe r : recipes) {
            ...
            for (int i = 0; i < sinv.length; i++) if (inv[i] != null) sinv[i] = inv[i].copy();
            if (!r.check(sinv)) continue;
            inv[cur] = r.output.copy();
            rs[cur] = r;
            cur++;
        }
        if (prev != null) for (int i = 27; i < 45; i++) if (rs[i] == prev) return i;
        return -1;
    }
}
