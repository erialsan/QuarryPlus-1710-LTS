...
public final class QuarryPlusUsageHandler extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(final String outputId, final Object... results) {
        if (outputId.equals("qpUsage") && getClass() == QuarryPlusUsageHandler.class)
            for (final Map.Entry<ItemStack, String[]> e : UsageList.getAll()
                .entrySet()) for (final String s : e.getValue()) this.arecipes.add(new Usage(e.getKey(), s));
        else super.loadCraftingRecipes(outputId, results);
    }

    @Override
    public void loadUsageRecipes(final ItemStack ingredient) {
        for (final String s : UsageList.getFromItemStack(ingredient)) this.arecipes.add(new Usage(ingredient, s));
    }
