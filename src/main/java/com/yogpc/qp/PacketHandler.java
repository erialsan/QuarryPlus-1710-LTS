...
public class PacketHandler extends SimpleChannelInboundHandler<YogpstopPacket> {
    ...
    protected void channelRead0(final ChannelHandlerContext ctx, final YogpstopPacket packet) throws Exception {
        if (packet.getChannel() == NBT) setNBTFromPacket(packet);
            ...
            final TileEntity t = packet.getPlayer().worldObj.getTileEntity(hdr.readInt(), hdr.readInt(), hdr.readInt());
        ...
        } else if (packet.getChannel() == KEY) QuarryPlus.proxy.setKeys(
            packet.getPlayer(),
            packet.getData()[0] << 24 | packet.getData()[1] << 16 | packet.getData()[2] << 8 | packet.getData()[3]);
    }
    ...
    public static void sendPacketToAround(final YogpstopPacket p, final int d, final int x, final int y, final int z) {
    ...
    private static void setNBTFromPacket(final YogpstopPacket p) {
        try {
            final NBTTagCompound cache = CompressedStreamTools
                .func_152457_a(p.getData(), NBTSizeTracker.field_152451_a);
            final TileEntity te = p.getPlayer().worldObj
                .getTileEntity(cache.getInteger("x"), cache.getInteger("y"), cache.getInteger("z"));
    ...
    public static void sendPacketToServer(final APacketTile te, final byte id, final byte pos, final String data) {// BLjava.lang.String;
    ...
    public static void sendNowPacket(final APacketTile te, final byte data) {
        sendPacketToAround(
            new YogpstopPacket(new byte[] { data }, te, StC_NOW),
    ...
    public static void sendPacketToAround(final APacketTile te, final byte id) {
        sendPacketToAround(
            new YogpstopPacket(new byte[0], te, id),
            te.getWorldObj().provider.dimensionId,
            te.xCoord,
            te.yCoord,
            te.zCoord);
