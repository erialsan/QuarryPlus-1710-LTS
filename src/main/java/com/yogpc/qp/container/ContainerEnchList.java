...
public class ContainerEnchList extends Container implements IPacketContainer {
    ...
    public boolean canInteractWith(final EntityPlayer ep) {
        return ep.getDistanceSq(this.tile.xCoord + 0.5D, this.tile.yCoord + 0.5D, this.tile.zCoord + 0.5D) <= 64.0D;
    }
    ...
    private static void writeList(final List<BlockData> l, final byte d, final DataOutputStream o) throws IOException {
    ...
    private static void writeDiff(final List<BlockData> a, final List<BlockData> b, final byte d1, final byte d2,
        final DataOutputStream o) throws IOException {
    ...
    public void receivePacket(final byte[] ba) {
        ...
        while ((slot = bb.readByte()) >= 0)
            if (slot == ADD_FORTUNE) this.tile.fortuneList.add(new BlockData(bb.readUTF(), bb.readInt()));
            else if (slot == ADD_SILKTOUCH) this.tile.silktouchList.add(new BlockData(bb.readUTF(), bb.readInt()));
            else if (slot == DEL_FORTUNE) this.tile.fortuneList.remove(new BlockData(bb.readUTF(), bb.readInt()));
            else if (slot == DEL_SILKTOUCH) this.tile.silktouchList.remove(new BlockData(bb.readUTF(), bb.readInt()));
