...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "cofh.api.energy.IEnergyContainerItem", modid = "CoFHAPI|energy"),
...
public class ItemArmorElectric extends ItemArmor implements ISpecialArmor, IElectricItem, IEnergyContainerItem {
    ...
    public ItemArmorElectric() {
        super(
            EnumHelper.addArmorMaterial("ELECTRIC", 33, new int[] { 3, 8, 6, 3 }, 0),
            QuarryPlus.proxy.addNewArmourRendererPrefix("electric"),
            1);
    ...
    private static final Field f = ReflectionHelper
        .getField(NetHandlerPlayServer.class, "field_147365_f", "floatingTickCount");
    ...
    private void useJetpack(final EntityPlayer p, final ItemStack jetpack, final boolean hover) {
        ...
        if (jumping || hover && p.motionY < -maxFallSpeedOnHover) {
            ...
            float power = 0.7f;
            if (charge / getMaxCharge(jetpack) <= 0.05f) power *= charge / getMaxCharge(jetpack) * 0.05;
            ...
            ElectricItemManager.discharge(jetpack, hover ? 7 : 8, getMaxCharge(jetpack));
            {
                ...
                if (p instanceof EntityPlayerMP) try {
    ...
    private static boolean toggleHover(final EntityPlayer p, final ItemStack itemStack) {
        ...
        if (QuarryPlus.proxy.getKey(p, ProxyCommon.Key.jump) && QuarryPlus.proxy.getKey(p, ProxyCommon.Key.mode)
            && toggleTimer == 0) {
            toggleTimer = 10;
            hoverMode = !hoverMode;
            if (!FMLCommonHandler.instance()
                ...
                if (hoverMode) p.addChatMessage(new ChatComponentTranslation("yog.armor.hover.on"));
                else p.addChatMessage(new ChatComponentTranslation("yog.armor.hover.off"));
            }
        }
        if (!FMLCommonHandler.instance()
            .getEffectiveSide()
            .isClient() && toggleTimer > 0) {
    ...
    public void onArmorTick(final World world, final EntityPlayer player, final ItemStack is) {
        if (ElectricItem.manager != null) ElectricItemManager.charge(
            is,
            ElectricItem.manager.discharge(is, Double.MAX_VALUE, Integer.MAX_VALUE, true, false, false),
            getMaxCharge(is));
        if (player.inventory.armorInventory[2] != is) return;
    ...
    public ISpecialArmor.ArmorProperties getProperties(final EntityLivingBase player, final ItemStack armor,
        ...
        final int damageLimit = (int) Math.min(Integer.MAX_VALUE, 25.0 * ElectricItemManager.getCharge(armor) / 5000);
        return new ISpecialArmor.ArmorProperties(0, 0.4 * 0.9, damageLimit);
    }

    @Override
    public int getArmorDisplay(final EntityPlayer player, final ItemStack armor, final int slot) {
        if (ElectricItemManager.getCharge(armor) >= 5000) return (int) Math.round(20.0 * 0.4 * 0.9);
        return 0;
    }

    @Override
    public void damageArmor(final EntityLivingBase entity, final ItemStack is, final DamageSource source,
    ...
    public int receiveEnergy(final ItemStack is, final int am, final boolean sim) {
        return (int) (ElectricItemManager.charge(is, Math.min((double) am / 4, getTransferLimit(is)), getMaxCharge(is))
            * 4);
    }
}
