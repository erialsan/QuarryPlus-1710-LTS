...
public class PowerManager {
    ...
    static void loadConfiguration(final Configuration cg) throws RuntimeException {
        final String cn = new StringBuilder().append(Configuration.CATEGORY_GENERAL)
            .append(Configuration.CATEGORY_SPLITTER)
            .append("PowerSetting")
            .append(Configuration.CATEGORY_SPLITTER)
            .toString();

        String cn2 = new StringBuilder().append(cn)
            .append("Quarry")
            .append(Configuration.CATEGORY_SPLITTER)
            .toString();
        ConfigCategory c = cg.getCategory(
            new StringBuilder().append(cn2)
                .append("BreakBlock")
                .toString());
        ...
        cn2 = new StringBuilder().append(cn)
            .append("Pump")
            .append(Configuration.CATEGORY_SPLITTER)
            .toString();
        c = cg.getCategory(
    ...
    private static void configure(final APowerTile pp, final double CE, final byte E, final byte U, final double CU,
        final double XR, final double MS, final byte pump) {
        pp.configure(
            XR * Math.pow(CE, E) / (U * CU + 1),
            MS * Math.pow(CE, E) / (U * CU + 1)
                + (pump > 0 ? 65536 * PL_BP / (pump * PL_CU + 1) + 1020 * PF_BP / (pump * PF_CU + 1) : 0));
    }

    private static void configure15(final APowerTile pp, final double CE, final byte E, final byte U, final double CU,
        final double XR, final double MS, final byte pump) {
        pp.configure(
            XR * Math.pow(CE, E) / (U * CU + 1),
            MS * Math.pow(CE, E) / (U * CU + 1)
    ...
    private static boolean useEnergy(final APowerTile pp, final double BP, final byte U, final double CU, final byte E,
        final double CE) {
