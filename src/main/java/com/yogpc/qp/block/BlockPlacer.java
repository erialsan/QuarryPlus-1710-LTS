...
public class BlockPlacer extends ADismCBlock {
    ...
    public void onNeighborBlockChange(final World par1World, final int par2, final int par3, final int par4,
        ...
        } else if (!flag && flag1) par1World.setBlockMetadataWithNotify(par2, par3, par4, i1 & -9, 4);
    }
        ...
        if (tile != null) {
            ...
            for (; i < tile.getSizeInventory(); i++) {
                ...
                if (is != null && is.getItem() != null) {
                    if (is.getItem()
                        .onItemUseFirst(is, player, world, tx, ty, tz, sd1, 0.5F, 0.5F, 0.5F)) break;
                    if (is.getItem()
                        .onItemUseFirst(
                            is,
                            player,
    ...
    public boolean onBlockActivated(final World world, final int x, final int y, final int z, final EntityPlayer ep,
        ...
        if (!world.isRemote) ep.openGui(QuarryPlus.I, QuarryPlusI.guiIdPlacer, world, x, y, z);
        return true;
    }
}
