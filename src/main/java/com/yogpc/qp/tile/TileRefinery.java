...
public class TileRefinery extends APowerTile implements IFluidHandler, IEnchantableTile {
    ...
    public void readFromNBT(final NBTTagCompound nbttc) {
        ...
        for (int i = 0; i < srcl.tagCount(); ++i) {
            ...
            if (j >= 0 && j < this.src.length) this.src[j] = FluidStack.loadFluidStackFromNBT(srct);
        }
    ...
    public void writeToNBT(final NBTTagCompound nbttc) {
        ...
        for (int i = 0; i < this.src.length; ++i) if (this.src[i] != null) {
        ...
        if (this.cached != null) nbttc.setTag("cached", this.cached.writeToNBT(new NBTTagCompound()));
    ...
    public void updateEntity() {
        ...
        if (this.rem_time > 0) this.rem_time--;
        if (this.rem_time > 0 || !PowerManager.useEnergyR(this, this.rem_energy, this.unbreaking, this.efficiency)) {
            decreaseAnimation();
            return;
        }
        increaseAnimation();
        if (this.res == null) this.res = this.cached.copy();
        else this.res.amount += this.cached.amount;
        this.cached = null;
        decreaseAnimation();
    ...
    private void simpleAnimationIterate() {
        if (this.animationSpeed > 1) {
            ...
            if (this.animationStage > 300) this.animationStage = 100;
        } else if (this.animationStage > 0) this.animationStage--;
    }
    ...
    private void sendNowPacket() {
        try {
            ...
            PacketHandler.sendPacketToAround(
                new YogpstopPacket(bos.toByteArray(), this, PacketHandler.StC_NOW),
                this.worldObj.provider.dimensionId,
                this.xCoord,
                this.yCoord,
                this.zCoord);
    ...
    private void increaseAnimation() {
        ...
        if (this.animationSpeed < 2) this.animationSpeed = 2;
        else if (this.animationSpeed <= 5) this.animationSpeed += 0.1;
    ...
    public FluidStack drain(final ForgeDirection from, final FluidStack resource, final boolean doDrain) {
        ...
        if (resource.isFluidEqual(this.res)) return drain(from, resource.amount, doDrain);
        for (int i = this.src.length - 1; i >= 0; i--) {
            ...
            if (doDrain) {
                ...
                if (this.src[i].amount == 0) this.src[i] = null;
            }
            RefineryRecipeHelper.get(this);
            return ret;
        }
    ...
    public FluidTankInfo[] getTankInfo(final ForgeDirection from) {
        ...
        for (int i = this.src.length - 1; i >= 0; i--) ret[i + 1] = new FluidTankInfo(this.src[i], this.buf);
        return ret;
    }
