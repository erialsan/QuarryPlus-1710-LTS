...
public class TileWorkbench extends APowerTile implements IInventory, IInventoryConnection {
    ...
    private static final ItemStack loadItemStackFromNBT(final NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("tag", 10)) r.setTagCompound(nbt.getCompoundTag("tag"));
        return r;
    }
    ...
    private static final void writeItemStackToNBT(final ItemStack is, final NBTTagCompound nbt) {
        ...
        if (is.stackTagCompound != null) nbt.setTag("tag", is.stackTagCompound);
    }
    ...
    public void updateEntity() {
        ...
        if (this.cur_recipe >= 0) {
            ...
            if (this.recipies[this.cur_recipe].power <= getStoredEnergy()) {
                ...
                for (int i = 0; i < 27; ++i) if (this.inv[i] != null && this.inv[i].stackSize <= 0) this.inv[i] = null;
    ...
    public void readFromNBT(final NBTTagCompound roottag) {
        ...
        for (int i = 0; i < islist.tagCount(); ++i) {
            ...
            if (j >= 0 && j < 27) this.inv[j] = loadItemStackFromNBT(istag);
        }
        WorkbenchRecipe.getRecipes(this.inv, this.recipies, -1);
        this.cur_recipe = roottag.getInteger("Recipe");
        if (this.cur_recipe >= 0 && this.recipies[this.cur_recipe] == null) this.cur_recipe = -1;
        if (this.cur_recipe >= 0) configure(250, this.recipies[this.cur_recipe].power * 2);
        else configure(250, 0);
    }
    ...
    public void writeToNBT(final NBTTagCompound roottag) {
        ...
        for (int i = 0; i < 27; ++i) if (this.inv[i] != null) {
    ...
    public boolean isUseableByPlayer(final EntityPlayer ep) {
        return this.worldObj.getTileEntity(this.xCoord, this.yCoord, this.zCoord) != this ? false
            : ep.getDistanceSq(this.xCoord + 0.5D, this.yCoord + 0.5D, this.zCoord + 0.5D) <= 64.0D;
    }
