...
public class ContainerMover extends Container implements IPacketContainer {
    ...
    public ContainerMover(final IInventory player, final World w, final int x, final int y, final int z) {
        ...
        int row;
        int col;
        for (col = 0; col < 2; ++col) addSlotToContainer(new SlotMover(this.craftMatrix, col, 8 + col * 144, 35));
        for (row = 0; row < 3; ++row) for (col = 0; col < 9; ++col)
            addSlotToContainer(new Slot(player, col + row * 9 + 9, 8 + col * 18, 84 + row * 18));
        for (col = 0; col < 9; ++col) addSlotToContainer(new Slot(player, col, 8 + col * 18, 142));
    }
    ...
    public void onContainerClosed(final EntityPlayer ep) {
        ...
        for (int var2 = 0; var2 < 2; ++var2) {
            ...
            if (var3 != null) ep.dropPlayerItemWithRandomChoice(var3, false);
        }
    }
    ...
    public ItemStack transferStackInSlot(final EntityPlayer ep, final int i) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            src = remain.copy();
            if (i < 2) {
                if (!mergeItemStack(remain, 2, 38, true)) return null;
            } else {
                Slot toslot;
            ...
            if (remain.stackSize == 0) slot.putStack(null);
    ...
    private void moveEnchant() {
        ...
        for (int i = 0; i < list.tagCount(); i++) {
            short lvl = list.getCompoundTagAt(i)
                .getShort("lvl");
            if (lvl > 0 && list.getCompoundTagAt(i)
                .getShort("id") == this.avail) {
                if (lvl > 1) list.getCompoundTagAt(i)
                    .setShort("lvl", --lvl);
                else {
                    {
                        final NBTTagList nlist = new NBTTagList();
                        for (int j = 0; j < list.tagCount(); j++) if (list.getCompoundTagAt(j)
                            .getShort("id") != this.avail) nlist.appendTag(list.getCompoundTagAt(j));
                        list = nlist;
                    }
                    is.getTagCompound()
                        .removeTag("ench");
                    if (list.tagCount() > 0) is.getTagCompound()
                        .setTag("ench", list);
                    if (is.getTagCompound()
                        .hasNoTags()) is.setTagCompound(null);
                }
                done = true;
                break;
            }
        ...
        } else for (int i = 0; i < list.tagCount(); i++) {
            final short id = list.getCompoundTagAt(i)
                .getShort("id");
            short lvl = list.getCompoundTagAt(i)
                .getShort("lvl");
    ...
    private int checkInventory() {
        ...
        if (this.craftMatrix.getStackInSlot(1) == null || pickaxeIs == null) return -1;
        final NBTTagList pickaxeE = pickaxeIs.getEnchantmentTagList();
        if (pickaxeE == null) return -1;
        for (int i = 0; i < pickaxeE.tagCount(); i++) {
            final short id = pickaxeE.getCompoundTagAt(i)
                .getShort("id");
            if (checkTo(id) && pickaxeE.getCompoundTagAt(i)
                .getShort("lvl") > 0 && (id == this.avail || this.avail < 0)) return id;
        }
        return -1;
    }
    ...
    private int next(final int add) {
        ...
        for (int i = add > 0 ? 0 : pickaxeE.tagCount() - 1; 0 <= i && i < pickaxeE.tagCount(); i += add) {
            final short id = pickaxeE.getCompoundTagAt(i)
                .getShort("id");
            if (!checkTo(id) || pickaxeE.getCompoundTagAt(i)
                .getShort("lvl") <= 0) continue;
            if (this.avail < 0 || next) return id;
            if (first < 0) first = id;
            if (id == this.avail) next = true;
        }
        return first;
    }
    ...
    private boolean checkTo(final int id) {
        ...
        if (target == null || !(target.getItem() instanceof IEnchantableItem)
            || !((IEnchantableItem) target.getItem()).canMove(target, id)) return false;
        final NBTTagList quarryE = target.getEnchantmentTagList();
        if (quarryE == null) return true;
        for (int i = 0; i < quarryE.tagCount(); i++) if (id == quarryE.getCompoundTagAt(i)
            .getShort("id")) {
                if (Enchantment.enchantmentsList[id].getMaxLevel() > quarryE.getCompoundTagAt(i)
                    .getShort("lvl")) return true;
                return false;
            }
        return true;
    }
