...
public class BlockController extends Block implements IDismantleable {
    ...
    private static final MobSpawnerBaseLogic getSpawner(final World w, final int x, final int y, final int z) {
        for (final ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            final TileEntity t = w.getTileEntity(x + d.offsetX, y + d.offsetY, z + d.offsetZ);
            if (!(t instanceof TileEntityMobSpawner)) continue;
    ...
    public boolean onBlockActivated(final World w, final int x, final int y, final int z, final EntityPlayer p,
        final int s, final float hx, final float hy, final float hz) {
        if (!w.isRemote) try {
            ...
            for (final Object e : EntityList.stringToClassMapping.entrySet()) {
                ...
                if (c == null || Modifier.isAbstract(c.getModifiers())
                    || QuarryPlusI.spawnerBlacklist.contains(ve.getKey())) continue;
                dos.writeByte(0);
            ...
            dos.writeInt(y);
            dos.writeInt(z);
            PacketHandler.sendPacketToPlayer(new YogpstopPacket(bos.toByteArray(), BlockController.class), p);
    ...
    public static void openGui(final byte[] ba) {
        ...
        switch (data.readByte()) {
            case 0:
                final MobSpawnerBaseLogic l = getSpawner(
                    DimensionManager.getWorld(data.readInt()),
                    data.readInt(),
                    data.readInt(),
                    data.readInt());
                if (l == null) return;
                final String tmp = data.readUTF();
                if (!QuarryPlusI.spawnerBlacklist.contains(tmp)) l.setEntityName(tmp);
                l.getSpawnerWorld()
                    .getTileEntity(l.getSpawnerX(), l.getSpawnerY(), l.getSpawnerZ())
                    .markDirty();
                l.getSpawnerWorld()
                    .markBlockForUpdate(l.getSpawnerX(), l.getSpawnerY(), l.getSpawnerZ());
                return;
            case 1:
                final List<String> s = new ArrayList<String>();
                while (data.readByte() >= 0) s.add(data.readUTF());
                FMLCommonHandler.instance()
                    .showGuiScreen(
                        QuarryPlus.proxy
                            .getGuiController(data.readInt(), data.readInt(), data.readInt(), data.readInt(), s));
        }
    }
    ...
    public void onNeighborBlockChange(final World w, final int x, final int y, final int z, final Block b) {
        ...
        final boolean r = w.isBlockIndirectlyGettingPowered(x, y, z) || w.isBlockIndirectlyGettingPowered(x, y + 1, z);
        ...
        } else if (!r && m == 1) w.setBlockMetadataWithNotify(x, y, z, 0, 4);
    }

    @Override
    public boolean canDismantle(final EntityPlayer arg0, final World arg1, final int arg2, final int arg3,
        final int arg4) {
        return true;
    }

    @Override
    public ArrayList<ItemStack> dismantleBlock(final EntityPlayer e, final World w, final int x, final int y,
        final int z, final boolean toinv) {
        final ArrayList<ItemStack> ret = getDrops(w, x, y, z, w.getBlockMetadata(x, y, z), 0);
        w.setBlockToAir(x, y, z);
        if (!toinv) for (final ItemStack is : ret) {
            final float f = 0.7F;
