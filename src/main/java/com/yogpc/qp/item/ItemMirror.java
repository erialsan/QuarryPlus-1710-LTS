    ...
    public ItemStack onEaten(final ItemStack i, final World w, final EntityPlayer p) {
        if (p instanceof EntityPlayerMP) {
            if (i.getItemDamage() == 2) {
                if (p.dimension != 0) p.travelToDimension(0);
            } else if (!p.worldObj.provider.canRespawnHere()) if (i.getItemDamage() == 1)
            ...
            if (c != null) c = EntityPlayer.verifyRespawnCoordinates(p.worldObj, c, p.isSpawnForced(p.dimension));
            else c = p.worldObj.provider.getRandomizedSpawnPoint();
