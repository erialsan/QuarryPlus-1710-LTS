...
public class WBPRecipeHandler extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(final String outputId, final Object... results) {
        if (outputId.equals("workbenchPlus") && getClass() == WBPRecipeHandler.class)
            for (final WorkbenchRecipe wbr : WorkbenchRecipe.getRecipes()) this.arecipes.add(new WBPRecipe(wbr));
    ...
    public void loadCraftingRecipes(final ItemStack result) {
        for (final WorkbenchRecipe wbr : WorkbenchRecipe.getRecipes())
            if (NEIServerUtils.areStacksSameTypeCrafting(wbr.output, result)) this.arecipes.add(new WBPRecipe(wbr));
    }
    ...
    public void loadUsageRecipes(final ItemStack ingredient) {
        for (final WorkbenchRecipe wbr : WorkbenchRecipe.getRecipes()) {
            ...
            if (recipe.contains(recipe.input, ingredient)) this.arecipes.add(recipe);
        }
    }
    ...
    public void drawExtras(final int recipeIndex) {
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(Double.toString(recipe.energy) + "MJ", 3, 121, 0x404040);
    }
    ...
    static {
        GuiContainerManager.addDrawHandler(new IContainerDrawHandler() {
            ...
            public void renderSlotUnderlay(final GuiContainer arg0, final Slot arg1) {
                if (arg0 instanceof GuiRecipe && ((GuiRecipe) arg0).getCurrentRecipeHandlers()
                    .get(((GuiRecipe) arg0).recipetype) instanceof WBPRecipeHandler && arg1.getHasStack())
