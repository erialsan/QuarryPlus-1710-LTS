...
public class TileQuarry extends TileBasic {
    ...
    private void S_updateEntity() {
        ...
        switch (this.now) {
            case MAKEFRAME:
                if (S_makeFrame()) while (!S_checkTarget()) S_setNextTarget();
                break;
            case MOVEHEAD:
                ...
                try {
                    ...
                    PacketHandler.sendPacketToAround(
                        new YogpstopPacket(bos.toByteArray(), this, PacketHandler.StC_HEAD_POS),
            ...
            case NOTNEEDBREAK:
            case BREAKBLOCK:
                if (S_breakBlock()) while (!S_checkTarget()) S_setNextTarget();
                break;
        }
        S_pollItems();
    }
    ...
    private boolean S_checkTarget() {
        ...
        final Block b = this.worldObj.getChunkProvider()
            .loadChunk(this.targetX >> 4, this.targetZ >> 4)
        ...
        switch (this.now) {
            ...
            case MAKEFRAME:
                ...
                if (b != null && b.getMaterial()
                    .isSolid()
                    && !(b == QuarryPlusI.blockFrame
                        && this.worldObj.getBlockMetadata(this.targetX, this.targetY, this.targetZ) == 0)) {
                ...
                if (flag > 1) {
                    if (b == QuarryPlusI.blockFrame
                        && this.worldObj.getBlockMetadata(this.targetX, this.targetY, this.targetZ) == 0) return false;
                    return true;
                }
                return false;
        }
    ...
    private double S_getDistance(final int x, final int y, final int z) {
        return Math
            .sqrt(Math.pow(x - this.headPosX, 2) + Math.pow(y + 1 - this.headPosY, 2) + Math.pow(z - this.headPosZ, 2));
    }
    ...
    private void S_checkDropItem() {
        final AxisAlignedBB axis = AxisAlignedBB.getBoundingBox(
            this.targetX - 4,
            this.targetY - 4,
            this.targetZ - 4,
            this.targetX + 6,
        ...
        for (int ii = 0; ii < result.size(); ii++) if (result.get(ii) instanceof EntityItem) {
                ...
                if (!S_checkIAreaProvider(this.xCoord, this.yCoord, this.zCoord - 1))
                    if (!S_checkIAreaProvider(this.xCoord, this.yCoord, this.zCoord + 1))
                        if (!S_checkIAreaProvider(this.xCoord, this.yCoord - 1, this.zCoord))
                            if (!S_checkIAreaProvider(this.xCoord, this.yCoord + 1, this.zCoord)) {
                                final ForgeDirection o = ForgeDirection.values()[this.worldObj
                                    .getBlockMetadata(this.xCoord, this.yCoord, this.zCoord)].getOpposite();
    ...
    private boolean S_checkIAreaProvider(final int x, final int y, final int z) {
        ...
        if (ModAPIManager.INSTANCE.hasAPI("BuildCraftAPI|core") && te instanceof IAreaProvider) {
            ...
            if (this.xCoord >= this.xMin && this.xCoord <= this.xMax
                && this.yCoord >= this.yMin
                && this.yCoord <= this.yMax
                && this.zCoord >= this.zMin
    ...
    public void G_renew_powerConfigure() {
        ...
        if (this.worldObj != null && this.pump != ForgeDirection.UNKNOWN) {
            final TileEntity te = this.worldObj.getTileEntity(
                this.xCoord + this.pump.offsetX,
                this.yCoord + this.pump.offsetY,
                this.zCoord + this.pump.offsetZ);
