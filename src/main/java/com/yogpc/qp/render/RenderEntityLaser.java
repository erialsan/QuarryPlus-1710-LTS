...
public class RenderEntityLaser extends Render {
    ...
    @Override
    public void doRender(final Entity e, final double i, final double j, final double k, final float f,
        final float f1) {
        if (e.isDead) return;
        GL11.glPushMatrix();
    ...
    static void doRender(final TextureManager tm, final double posX, final double posY, final double posZ,
        final double headX, final double headY, final double headZ, final double armXSize, final double armZSize) {
    ...
    private static void doRender(final TextureManager tm, final double i, final double j, final double k,
        ...
        for (int iBase = 0; iBase < iSize; ++iBase)
            for (int jBase = 0; jBase < jSize; ++jBase) for (int kBase = 0; kBase < kSize; ++kBase) {
                ...
                rb.setRenderBounds(
                    ...
                    0,
                    0,
                    remainX > 1.0 ? 1.0 : remainX,
                    remainY > 1.0 ? 1.0 : remainY,
