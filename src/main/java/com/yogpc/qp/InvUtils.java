...
public class InvUtils {
    ...
    public static final int addToIInv(final IInventory ii, final ItemStack is1, final ForgeDirection fd,
        final boolean doAdd) {
        IInventory iii;
        int[] a;
        ...
        if (iii instanceof ISidedInventory) a = ((ISidedInventory) iii).getAccessibleSlotsFromSide(fd.ordinal());
        else {
        ...
        for (final int i : a) {
            if (iii instanceof ISidedInventory) {
                if (!((ISidedInventory) iii).canInsertItem(i, is1, fd.ordinal())) continue;
            } else if (!iii.isItemValidForSlot(i, is1)) continue;
            ...
            buf = Math.min(iii.getInventoryStackLimit(), Math.min(is2.stackSize + rem, is2.getMaxStackSize()));
            if (buf > is2.stackSize) {
                rem -= buf - is2.stackSize;
                if (doAdd) is2.stackSize = buf;
                if (rem <= 0) return is1.stackSize;
            }
        }
    ...
    public static void injectToNearTile(final World w, final int x, final int y, final int z, final ItemStack is) {
